+++
title = "GMKtec NucBox5 Benchmarks"
date = "2024-04-27"
...
{{< callout "tip" >}}Update (Apr 2026): It's now been over two years, and it's still running reliably. Power efficiency remains excellent, but the Celeron N5105 is starting to show its age for heavier workloads.{{< /callout >}}
...
{{< callout "note" >}}Performance is in line with low-end modern CPUs, fine for general use, but slow for sustained or heavier workloads.{{< /callout >}}
...
{{< callout "note" >}}SSD performance is adequate, but clearly budget-tier good enough for everyday use, not heavy I/O.{{< /callout >}}
...
{{< callout "note" >}}GPU is sufficient for desktop compositing and light workloads, but not suited for modern gaming.{{< /callout >}}
...
{{< callout "note" >}}Small models are usable, but anything above ~1–1.5B parameters becomes impractically slow on CPU.{{< /callout >}}
...
{{< callout "important" >}}System used around 16.6 to 19 W during inference.{{< /callout >}}
...
{{< callout "note" >}}Score is subjective: 1 = unplayable, 5 = playable with issues, 10 = perfect{{< /callout >}}
